package sweepers

import (
	"errors"
	"fmt"

	"github.com/scaleway/scaleway-sdk-go/api/instance/v1"
	"github.com/scaleway/scaleway-sdk-go/logger"
	"github.com/scaleway/scaleway-sdk-go/scw"
)

func SweepServers(scwClient *scw.Client, zone scw.Zone) error {
	instanceAPI := instance.NewAPI(scwClient)
	logger.Warningf("sweeper: destroying instance servers in %q", zone)

	listServers, err := instanceAPI.ListServers(&instance.ListServersRequest{
		Zone: zone,
	}, scw.WithAllPages())
	if err != nil {
		return fmt.Errorf("error listing servers: %w", err)
	}

	errs := []error(nil)
	for _, srv := range listServers.Servers {
		switch srv.State {
		case instance.ServerStateStopped, instance.ServerStateStoppedInPlace:
			err := instanceAPI.DeleteServer(&instance.DeleteServerRequest{
				Zone:     zone,
				ServerID: srv.ID,
			})
			if err != nil {
				errs = append(errs, fmt.Errorf("error deleting server %s: %w", srv.ID, err))
			}

		case instance.ServerStateRunning:
			_, err := instanceAPI.ServerAction(&instance.ServerActionRequest{
				Zone:     zone,
				ServerID: srv.ID,
				Action:   instance.ServerActionTerminate,
			})
			if err != nil {
				errs = append(errs, fmt.Errorf("error terminating server %s: %w", srv.ID, err))
			}
		}
	}

	return errors.Join(errs...)
}

func SweepVolumes(scwClient *scw.Client, zone scw.Zone) error {
	instanceAPI := instance.NewAPI(scwClient)
	logger.Warningf("sweeper: destroying instance volumes in %q", zone)

	listVolumesResponse, err := instanceAPI.ListVolumes(&instance.ListVolumesRequest{
		Zone: zone,
	}, scw.WithAllPages())
	if err != nil {
		return fmt.Errorf("error listing instance volumes: %w", err)
	}

	errs := []error(nil)
	for _, volume := range listVolumesResponse.Volumes {
		if volume.Server == nil {
			err := instanceAPI.DeleteVolume(&instance.DeleteVolumeRequest{
				Zone:     zone,
				VolumeID: volume.ID,
			})
			if err != nil {
				errs = append(errs, fmt.Errorf("error deleting instance volume %s: %w", volume.ID, err))
			}
		}
	}
	return errors.Join(errs...)
}

func SweepSnapshots(scwClient *scw.Client, zone scw.Zone) error {
	api := instance.NewAPI(scwClient)
	logger.Warningf("sweeper: destroying instance snapshots in %q", zone)

	listSnapshotsResponse, err := api.ListSnapshots(&instance.ListSnapshotsRequest{
		Zone: zone,
	}, scw.WithAllPages())
	if err != nil {
		return fmt.Errorf("error listing instance snapshots: %w", err)
	}

	errs := []error(nil)
	for _, snapshot := range listSnapshotsResponse.Snapshots {
		err := api.DeleteSnapshot(&instance.DeleteSnapshotRequest{
			Zone:       zone,
			SnapshotID: snapshot.ID,
		})
		if err != nil {
			errs = append(errs, fmt.Errorf("error deleting instance snapshot %s: %w", snapshot.ID, err))
		}
	}

	return errors.Join(errs...)
}

func SweepSecurityGroups(scwClient *scw.Client, zone scw.Zone) error {
	instanceAPI := instance.NewAPI(scwClient)
	logger.Warningf("sweeper: destroying instance security groups in %q", zone)

	listResp, err := instanceAPI.ListSecurityGroups(&instance.ListSecurityGroupsRequest{
		Zone: zone,
	}, scw.WithAllPages())
	if err != nil {
		return fmt.Errorf("error listing security groups: %w", err)
	}

	errs := []error(nil)
	for _, securityGroup := range listResp.SecurityGroups {
		// Can't delete default security group.
		if securityGroup.ProjectDefault {
			continue
		}
		err = instanceAPI.DeleteSecurityGroup(&instance.DeleteSecurityGroupRequest{
			Zone:            zone,
			SecurityGroupID: securityGroup.ID,
		})
		if err != nil {
			errs = append(errs, fmt.Errorf("error deleting security group %s: %w", securityGroup.ID, err))
		}
	}

	return errors.Join(errs...)
}

func SweepPlacementGroups(scwClient *scw.Client, zone scw.Zone) error {
	instanceAPI := instance.NewAPI(scwClient)
	logger.Warningf("sweeper: destroying instance placement groups in %q", zone)

	listPlacementGroups, err := instanceAPI.ListPlacementGroups(&instance.ListPlacementGroupsRequest{
		Zone: zone,
	}, scw.WithAllPages())
	if err != nil {
		return fmt.Errorf("error listing placement groups: %w", err)
	}

	errs := []error(nil)
	for _, pg := range listPlacementGroups.PlacementGroups {
		err := instanceAPI.DeletePlacementGroup(&instance.DeletePlacementGroupRequest{
			Zone:             zone,
			PlacementGroupID: pg.ID,
		})
		if err != nil {
			errs = append(errs, fmt.Errorf("error deleting placement group %s: %w", pg.ID, err))
		}
	}

	return errors.Join(errs...)
}

func SweepIP(scwClient *scw.Client, zone scw.Zone) error {
	instanceAPI := instance.NewAPI(scwClient)
	logger.Warningf("sweeper: destroying instance ips in %q", zone)

	listIPs, err := instanceAPI.ListIPs(&instance.ListIPsRequest{
		Zone: zone,
	}, scw.WithAllPages())
	if err != nil {
		return fmt.Errorf("error listing instance ips: %w", err)
	}

	errs := []error(nil)
	for _, ip := range listIPs.IPs {
		err := instanceAPI.DeleteIP(&instance.DeleteIPRequest{
			IP:   ip.ID,
			Zone: zone,
		})
		if err != nil {
			errs = append(errs, fmt.Errorf("error deleting instance ip %s: %w", ip.ID, err))
		}
	}

	return errors.Join(errs...)
}

func SweepImages(scwClient *scw.Client, zone scw.Zone) error {
	api := instance.NewAPI(scwClient)
	logger.Warningf("sweeper: destroying instance images in %q", zone)

	listImagesResponse, err := api.ListImages(&instance.ListImagesRequest{
		Zone:   zone,
		Public: scw.BoolPtr(false),
	}, scw.WithAllPages())
	if err != nil {
		return fmt.Errorf("error listing instance images: %w", err)
	}

	errs := []error(nil)
	for _, image := range listImagesResponse.Images {
		err := api.DeleteImage(&instance.DeleteImageRequest{
			Zone:    zone,
			ImageID: image.ID,
		})
		if err != nil {
			errs = append(errs, fmt.Errorf("error deleting instance image %s: %w", image.ID, err))
		}
	}

	return errors.Join(errs...)
}

func SweepAllLocalities(scwClient *scw.Client) error {
	errs := []error(nil)
	for _, zone := range (&instance.API{}).Zones() {
		err := SweepServers(scwClient, zone)
		if err != nil {
			errs = append(errs, err)
		}
		err = SweepVolumes(scwClient, zone)
		if err != nil {
			errs = append(errs, err)
		}
		err = SweepSnapshots(scwClient, zone)
		if err != nil {
			errs = append(errs, err)
		}
		err = SweepSecurityGroups(scwClient, zone)
		if err != nil {
			errs = append(errs, err)
		}
		err = SweepPlacementGroups(scwClient, zone)
		if err != nil {
			errs = append(errs, err)
		}
		err = SweepIP(scwClient, zone)
		if err != nil {
			errs = append(errs, err)
		}
		err = SweepImages(scwClient, zone)
		if err != nil {
			errs = append(errs, err)
		}
	}

	return errors.Join(errs...)
}
